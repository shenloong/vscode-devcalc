...
import { config, langs, devcalcRegExp } from './configuration';
...
function setAnnotations(line: number) {
    ...
    const devValues = lineText.match(new RegExp(devcalcRegExp.dev, 'ig'));
    if (!devValues) return;
    // check if there are any px and dev values in the line
    const lineValues = lineText.match(new RegExp(devcalcRegExp.decoration, 'ig'));
    if (!lineValues) return;
    return lineValues.map(annotation).join(' ');
}

function annotation(value: string) {
    return new RegExp(devcalcRegExp.dev, 'ig').test(value) ? dev2px(value).pxText : value.trim().replace(':', '').replace(';', '');
}
