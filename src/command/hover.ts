...
import { config, numericRegExp, devcalcRegExp } from './configuration';
import { dev2px } from './conversion';

export const dev2pxHover = {
    provideHover(document: TextDocument, position: Position) {
        const devTextRange = document.getWordRangeAtPosition(position, new RegExp(devcalcRegExp.dev, 'ig'));
        if (!devTextRange) return;
        const devText = document.getText(devTextRange);
        if (!devText) return;
        const devUnit = devText.replace(new RegExp(numericRegExp, 'g'), '');
